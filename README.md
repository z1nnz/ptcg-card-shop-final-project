...
- `PATCH /api/admin/orders/:id/status`：管理員更新訂單狀態
- `PATCH /api/admin/products/:id`：管理員更新商品價格與庫存
...
```text
user@ptcg.com / user123
```

4. 從商品頁加入購物車，進入購物車並完成結帳。

5. 進入會員專區，確認「訂單查詢」可以看到剛建立的訂單。

6. 登出後使用管理員帳號登入。

```text
admin@ptcg.com / admin123
```

7. 進入管理員面板，確認會員數、訂單數、總營收與訂單列表都有更新。

8. 在管理員面板將訂單狀態改為「已出貨」或「已完成」，回會員專區確認訂單狀態同步更新。

9. 在商品管理調整某項商品庫存，確認商品資料由 SQLite 後端維護。

10. 到留言板新增留言，回到首頁確認留言統計會增加。
